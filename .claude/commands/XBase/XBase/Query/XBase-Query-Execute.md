...
6. Dispatch by `Operation`:
   - **SELECT**: Read all lines from `{TableName}.dbf`; parse each as JSON; apply `IncludeDeleted` filter; apply `Filter` specification in memory; if `Join` is supplied, read the joined table's `.dbf` and merge rows on the key columns (INNER: drop non-matching; LEFT: keep all primary rows with nulls for missing secondary); compute `TotalCount`; apply `Aggregate` if supplied; apply `Sort`; apply `Offset` then `Limit`; project `Columns`; return `Rows` and `TotalCount`.
   - **INSERT**: Enforce NOT NULL, UNIQUE, and FK constraints in memory against `_schema.json` and the existing `.dbf`; assign `Id` from `NextId`; set `CreatedAt`, `UpdatedAt`, `IsDeleted`; append a JSON line to `<TableName>.dbf`; update index `.ndx` files; write updated `_schema.json`; return `InsertedCount` and `LastInsertedId`.
   - **UPDATE**: Require `Filter`; read `<TableName>.dbf`; apply Filter in memory; apply `Values` to each matching row; enforce constraints; set `UpdatedAt`; rewrite `<TableName>.dbf`; update index `.ndx` files; return `AffectedRows`.
   - **DELETE**: Require `Filter`; read `<TableName>.dbf`; apply Filter; if `HardDelete` is `true`, rewrite the file omitting matching lines; otherwise set `IsDeleted=1` and update `UpdatedAt` for matching rows; update index `.ndx` files; return `AffectedRows`.
