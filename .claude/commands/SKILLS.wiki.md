...
XBase has no external runtime dependencies. It reads and writes plain files using OS file system primitives. When a skill executes, it detects the available runtime environment and generates the appropriate script:

| Environment | Detection | Script type generated |
|---|---|---|
| Windows PowerShell / pwsh | `$PSVersionTable` present | PowerShell (`.ps1`) |
| bash / zsh (Linux, macOS) | `$BASH_VERSION` or `$ZSH_VERSION` present | POSIX shell (`#!/usr/bin/env bash`) |
| Python fallback | `python3` or `python` on PATH | Python 3 script (stdlib `os`, `json`, `shutil` only) |

No installation step is required. The AI generates the correct script inline during skill execution based on what is available in the deployment environment.
...
A lightweight file-based database engine accessed entirely through skills. No external dependencies — skills detect the runtime environment (PowerShell, bash, or Python) and generate the appropriate file system operations on-demand. 29 skills across 8 operation groups.

See [XBase.wiki.md](XBase/XBase.wiki.md) for full documentation.

| Group | Skills | Purpose |
|---|---|---|
| Runtime | 1 | Detect execution environment; choose PowerShell, bash, or Python |
| Database | 4 | Lifecycle: initialize, connect, disconnect, drop |
| Schema | 5 | Table and column DDL |
| Record | 5 | CRUD + upsert |
| Query | 5 | Filters, sorts, joins, aggregates, compound execution |
...
The `SKILLS/` folder is the GitHub ZIP distribution artifact. Its directory structure mirrors the skill naming convention exactly. Files are plain Markdown — no compiled output, no external runtime dependencies. All database operations are performed through OS file system primitives; the required script (PowerShell, bash, or Python) is generated dynamically by the AI at execution time.
