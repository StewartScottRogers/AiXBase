...
4. Validate each name in `Columns` against the table's column definitions; return `XBASE_SCHEMA_COLUMN_MISSING` for any unknown column name. Skip this check if `["*"]` is present.
